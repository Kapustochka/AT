...
# def tests
  ...
  tc('tc_02', 'login_with_correct_creds') do
    ...
    unless @home_page.has_text?('Signed in successfully.')
      raise "\tExpected text '#{@home_page.text}' to include 'Signed in successfully.'"
    end
  end
  ...
  tc('tc_03', 'logout is working') do
    ...
    unless @home_page.has_text?('Signed out successfully.')
      raise "\tExpected text '#{@home_page.text}' to include 'Signed out successfully.'"
    end
  end
  ...
  tc('tc_04_1', 'prohib_log_without_passwd') do
    ...
    unless @login_page.has_text?('Invalid email or password.')
      raise "\tExpected text '#{@login_page.text}' to include 'Invalid email or password.'"
    end
  end
  ...
  tc('tc_05_2', 'prohib_log_with_inc_passwd') do
    ...
    @login_page.login_button.click
    unless @login_page.has_text?('Invalid email or password.')
      raise "\tExpected text '#{page.text}' to include 'Invalid email or password.'"
    end
  end
